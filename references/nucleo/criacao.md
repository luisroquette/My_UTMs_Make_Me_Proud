...
- When the normalized slug exceeds 80 chars, truncate it to `80 - len(hash)` and append a short deterministic hash — never truncate without the hash, plain truncation creates collisions between similar names. The hash is FNV-1a (32-bit, base36, ~7 chars) computed over the **entire normalized slug** (not the truncated prefix, or long names sharing a prefix would still collide). The final slug must stay within 1-80 chars.
